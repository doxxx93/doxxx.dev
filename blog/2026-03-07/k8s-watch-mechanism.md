...
etcd는 MVCC(Multi-Version Concurrency Control) 모델을 사용합니다. 모든 쓰기 작업은 단조 증가하는 **revision** 번호를 부여받으며, 이전 상태가 즉시 삭제되지 않고 보존됩니다. 덕분에 "revision 1500 이후에 바뀐 것만 알려줘" 같은 요청이 가능하고, Watch가 정확히 이 방식으로 동작합니다. Kubernetes의 `resourceVersion`이 바로 이 etcd revision입니다.

다만 이전 상태가 영원히 남는 건 아닙니다. **Compaction**이 주기적으로 오래된 revision을 날리기 때문입니다 (kubeadm 기본값 5분). 날아간 revision은 더 이상 조회할 수 없고, 이게 뒤에서 다룰 `410 Gone`의 원인 중 하나입니다.
...
Watch Cache 안에서 실제로 이 일을 하는 건 **Cacher**입니다. Pod, Service, Secret 등 리소스 타입마다 독립된 Cacher 인스턴스가 있고, 각각이 etcd에서 이벤트를 받아(cacherListerWatcher → Reflector) Ring Buffer에 저장한 뒤(watchCache) 등록된 watcher들에게 분배합니다(dispatchEvents).

Cacher 내부의 **Ring Buffer**는 현재 상태 전체가 아니라 변경 이벤트(delta)만 저장합니다. "Pod A 생성됨", "Pod B status 변경" 같은 이벤트가 쌓이는 구조입니다. 크기는 동적으로 조절되며 대략 75초 분량의 히스토리를 유지합니다. 새 watcher가 중간에 합류해도 최근 변경분은 받을 수 있지만, 75초를 넘긴 이벤트는 복구할 수 없습니다 — `410 Gone`이 여기서 발생합니다.

각 Cacher는 리소스별로 독립적인 resourceVersion을 추적합니다. Pod처럼 변경이 많은 리소스는 RV가 빠르게 전진하지만, RuntimeClass처럼 한번 설정하고 거의 안 바뀌는 리소스는 RV가 오랫동안 정체됩니다. 글로벌 RV는 50000인데 특정 리소스의 RV는 45000에 머물러 있는 식입니다. 이 격차가 뒤에서 다룰 Quiet Resource Timeout으로 이어집니다.
...
클라이언트 쪽에서는 **Reflector**가 List-Watch 루프를 실행합니다. 처음 시작할 때는 현재 상태를 전혀 모르기 때문에 LIST로 전체 스냅샷을 가져옵니다. 이후에는 그 시점의 resourceVersion부터 WATCH를 걸어서 변경분만 수신합니다. 같은 리소스를 보는 핸들러가 여럿이면 **SharedInformer**가 하나의 watch 연결을 공유시켜 API Server 부하를 줄입니다.

연결이 끊기면 마지막으로 알고 있는 resourceVersion으로 자동 재연결합니다. 이 재연결이 실패하면 — Ring Buffer에 해당 RV가 남아있지 않으면 — `410 Gone`이 발생합니다.
...
리더 락 갱신까지 실패하면 리더십 전환이 일어나고, 새 리더의 Informer가 다시 relist하면서 부하가 더 쌓입니다. 수천 노드 프로덕션 클러스터에서 실제로 수 분간 마비된 사례가 있습니다 ([#86483](https://github.com/kubernetes/kubernetes/issues/86483)).
...
v1.17의 Bookmark 이벤트([KEP-956](https://github.com/kubernetes/enhancements/blob/master/keps/sig-api-machinery/956-watch-bookmark/README.md))가 변경이 없어도 현재 RV를 클라이언트에 알려줘서 410 발생 자체를 줄였고, v1.19의 List Semantic 변경([#86430](https://github.com/kubernetes/kubernetes/issues/86430))으로 410 이후 relist도 Watch Cache에서 응답할 수 있게 되면서 etcd 직접 요청이 크게 줄었습니다.

클라이언트 쪽에서도 Reflector의 고정 1초 재시도가 지수 백오프 + 랜덤 지연으로 바뀌면서([#87794](https://github.com/kubernetes/kubernetes/issues/87794)) 동시 Relist가 약 98% 감소했습니다. v1.24의 Efficient Watch Resumption([KEP-1904](https://github.com/kubernetes/enhancements/issues/1904))은 API Server 재시작 후에도 Watch Cache를 빠르게 복구해줍니다.

운영 측면에서는 K8s 1.19 이상이어야 합니다. 롤링 업그레이드 간격은 2~3분 확보하고, `--watch-cache-sizes`를 늘리고 `--goaway-chance`로 watch 연결을 분산시킵니다.
...
원래는 WatchList([KEP-3157](https://github.com/kubernetes/enhancements/blob/master/keps/sig-api-machinery/3157-watch-list/README.md))가 LIST + Watch를 단일 스트림으로 통합하는 접근이었지만, v1.33에서 기본 비활성화로 돌아갔습니다. 대신 Streaming Encoder([StreamingCollectionEncoding](https://kubernetes.io/blog/2024/12/17/kube-apiserver-api-streaming/))가 기존 LIST API를 유지하면서 항목별 스트리밍 인코딩을 도입해 메모리를 ~20배 줄였습니다.
...
운영 측면에서는 K8s 1.33+에서 StreamingCollectionEncoding이 기본 활성화되어 있는지 확인하고, 메모리 limit을 충분히 (16GB+) 잡고, 컨트롤러 동시 재시작을 피합니다.
...
문제는 **에러가 안 난다**는 겁니다. `rv=0` 요청이 stale cache에서 오래된 데이터를 반환하고, 같은 명령을 반복하면 성공과 실패가 번갈아 나타납니다.

Webhook이 "service not found"를 반환하거나, Pod 환경변수에서 Service 정보가 빠지는 등 증상이 다양합니다.
보통 5~10분이면 cache가 따라잡으면서 풀리지만, 그 사이에 같은 Pod가 두 노드에서 동시에 뜨면 StatefulSet 데이터가 손상될 수 있습니다.
...
Watch Cache는 etcd 앞의 단순 버퍼에서 시작해, 지금은 거의 모든 읽기를 처리하는 레이어가 됐습니다. 위 시나리오들은 대부분 1.24 이상에서 크게 완화되고, 1.31+에서는 읽기 경로까지 캐시로 옮겨가면서 etcd 부하 자체가 줄었습니다.
