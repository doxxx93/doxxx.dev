...
[PR #1836](#pr-1836)을 올렸다. 파일 체인지가 1개라 부담이 없었다. 메인테이너 clux의 첫 리뷰:
...
개발 흐름에 대한 이해가 생겼다. 이 구현을 끝낼 쯤 clux가 후속 이슈를 만들어줬고, [PR #1838](#pr-1838)로 자연스럽게 이어졌다.

### 이슈부터 시작하기

첫 PR은 의논 없이 바로 올렸지만, [PR #1838](#pr-1838)
...
이슈에 먼저 리서치 결과를 남기고, 2단계로 나눠 [PR #1873](#pr-1873)
과 [PR #1876](#pr-1876)을 올렸다. clux가 느린 클러스터에서 직접 테스트한 결과:
...
분석을 다시 검증하고 [PR #1882](#pr-1882)를 올렸다. 솔직히 경쟁심도 있었다. 제시된 설계가 좀 과한 느낌이었고, 더 단순하게 짤 수 있을
것 같았다. 기여의 동기가 꼭 고상할 필요는 없다고 생각한다. 회사 코드 개선, 호기심, 경쟁심, 어떤 것이든 동기가 다양할수록 오래 하게 되는 것 같다.

### 릴리즈와 버전

[PR #1884](#pr-1884)는 subresource 메서드의 API를 개선하는 breaking change였는데, 마침
3.0.0 마일스톤 타이밍에 맞아서 포함될 수 있었다.

[PR #1936](#pr-1936)은 dependabot이 올린 rand 0.10 업데이트 PR이 breaking API change를 처리하지 않아
...
이 이슈를 기반으로 [PR #1894](#pr-1894)를 작업했는데, 충분히 고민해서 구현한 결과물에 대해 clux가 tower의
...
기능을 만들려고 주변 코드를 읽다 보니 기존 버그가 보이는 식이었다. [PR #1897](#pr-1897)로 수정했다.
...
않고 [PR #1908](#pr-1908)을 올렸다. 해당 PR의 변경 범위가 넓다고 느꼈고 더 작게 고칠 수 있다고 생각했는데, 돌이켜보면
...
정리하거나([PR #1930](#pr-1930)), Docker Hub rate limit으로 flaky해지는 CI를
고치는([PR #1913](#pr-1913)) 작업들이었다. 화려하지는 않지만 workaround를 제대로 고치는 재미가 있었다.

[PR #1937](#pr-1937)은 memory benchmark CI였다. 회사 제품에서도 버전이 올라갈 때마다 성능 추적이 필요하다고 느끼고
...
이런 CI 작업들을 하면서, 앞서 의존성 하한값 문제에서 느꼈던 것도 다시 떠올랐다. 결국 [PR #1940](#pr-1940)으로 이어졌다.
...
- **해결**: 캐시 키에 UID를 포함하는 `PredicateCacheKey` 도입 — [#1836](https://github.com/kube-rs/kube/pull/1836)
...
- **해결**: 설정 가능한 TTL이 있는 Config 구조체 추가, polling마다 만료 엔트리 자동 제거 — [#1838](https://github.com/kube-rs/kube/pull/1838)
...
- **해결**: Aggregated Discovery API 타입 정의 및 Client 메서드 추가 — [#1873](https://github.com/kube-rs/kube/pull/1873)
...
- **해결**: `Discovery::run_aggregated()` 메서드 추가, v2 응답을 내부 타입으로 변환 — [#1876](https://github.com/kube-rs/kube/pull/1876)
...
- **해결**: `WatchPhase` enum으로 초기 watch와 재연결 구분, 재연결 시 initial events 요청 안 함 — [#1882](https://github.com/kube-rs/kube/pull/1882)
...
- **해결**: `Vec<u8>` 대신 `&K where K: Serialize`로 시그니처 변경 — [#1884](https://github.com/kube-rs/kube/pull/1884)
...
- **해결**: `tower::retry::Policy`를 구현하는 `RetryPolicy` 추가. 지수 백오프 적용 — [#1894](https://github.com/kube-rs/kube/pull/1894)
...
- **해결**: 반환값을 builder에 재할당하여 jitter 정상 동작 — [#1897](https://github.com/kube-rs/kube/pull/1897)
...
- **해결**: `o.len() == 1` 필터 제거, `anyOf` 패턴 매칭 조건만으로 충분 — [#1908](https://github.com/kube-rs/kube/pull/1908)
...
- **해결**: busybox 태그를 `stable`로 통일, k3d에 이미지를 미리 import — [#1913](https://github.com/kube-rs/kube/pull/1913)
...
- **해결**: 불필요한 lint suppression 제거, 일부는 실제 경고를 수정 — [#1930](https://github.com/kube-rs/kube/pull/1930)
...
- **해결**: `rand::Rng` → `rand::RngExt` 이름 변경 적용 (dev-dependency만 해당) — [#1936](https://github.com/kube-rs/kube/pull/1936)
...
- **해결**: memory.rs 벤치마크의 JSON 출력 + GitHub Actions 워크플로우 추가, 150% 초과 시 PR 코멘트 — [#1937](https://github.com/kube-rs/kube/pull/1937)
...
- **해결**: `cargo +nightly update -Z minimal-versions`로 최소 버전 resolution 후 빌드 검증 CI 추가 — [#1940](https://github.com/kube-rs/kube/pull/1940)
